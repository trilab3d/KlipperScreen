...
class PrintDetail(BaseWizardStep):
    ...
    def activate(self, wizard):
        ...
        box2.set_halign(Gtk.Align.START)
        box2.set_orientation(Gtk.Orientation.HORIZONTAL)
        lbl = self._screen.gtk.Label(self.wizard_manager.format_time(fileinfo["estimated_time"]) if "estimated_time" in fileinfo else "---")
        box2.add(lbl)
        box.add(box2)
        box.set_margin_start(12)
...
class MismatchDetected(BaseWizardStep):
    ...
    def activate(self, wizard):
        ...
        if filament_type != filament_loaded:
            ...
            lbl.set_markup("<span bgcolor='#008800'>" + self.wizard_manager.get_wizard_data("filament_type") + "</span>")
            box.add(lbl)
            box.set_margin_start(12)
            grid.attach(box, 0, 1, 1, 1)
        ...
        if nozzle_current != nozzle_wanted:
            ...
            box.add(self._screen.gtk.Label(_("Nozzle"), "property-name", xalign=0.0))
            lbl = self._screen.gtk.Label("", xalign=0.0)
            lbl.set_markup("<span bgcolor='#008800'>" + self.wizard_manager.get_wizard_data("nozzle_wanted") + "</span>")
            box.add(lbl)
            box.set_margin_start(12)
            grid.attach(box, 0, 2, 1, 1)
